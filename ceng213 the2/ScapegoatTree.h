...
ScapegoatTree<T>::ScapegoatTree(const ScapegoatTree<T> &obj) {
    ...
    lhs->element = obj.root->element;
    lhs->left = deep_copy(obj.root->left);
    lhs->right = deep_copy(obj.root->right);
    root = lhs;
    upperBound = root->element;
}
...
template<class T>      
Node<T>* ScapegoatTree<T>::findMinValue(Node<T> *node) {
    Node<T>* curr = node;
